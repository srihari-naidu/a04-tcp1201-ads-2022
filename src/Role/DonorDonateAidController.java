...
/**
 * FXML Controller class for Donor Donate Aid.
 */
public class DonorDonateAidController extends DonorLoginController {
    ...
    @FXML
    void btnDonateClicked(ActionEvent event) {
        if (validateDonation()) {
            donateAid(event);
            super.closeWindow(event);
            super.switchToDonorHome(event);
        }
    }
    ...
    private boolean validateAidQuantity() {
        ...
        for (int i = 0; i < aidQuantity.length(); i++) {
            if (!(aidQuantity.charAt(i) >= '0' && aidQuantity.charAt(i) <= '9')) {
                alertMsg = alertMsg + "Aid quantity must be a number.\n";
                return false; 
            }
        }
        return true;
    ...
    private boolean validateDonation() {
        if (validateInput() == true) {
            ...
            if (valAN && valAQ) {
                ...
                alert.setHeaderText("Aid Donation Failed!");
    ...
    /** 
     * Adds the donated aid to the list of existing aids and saves it in its .csv datafile.
     *
     * @param event
     */
    private void donateAid(ActionEvent event) {
        String aidName = txtAidName.getText().trim();
        int aidQuantity = Integer.parseInt(txtAidQuantity.getText().trim());

        ArrayList<Aid> aids = Aid.getAids();
        Aid donatedAid = new Aid(aidName, aidQuantity, user.getID(), "-", "Available");
        boolean plusAid = false;

        for (Aid aid : aids) {
            if (aid.equals(donatedAid)) {
                aid.setQuantity(aid.getQuantity() + donatedAid.getQuantity());
                plusAid = true;
            }
        }
